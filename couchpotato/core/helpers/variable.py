...
def _keyword_password_value(name, marker):
...
def _keyword_password_prefix_bounds(name, marker):
    ...
    return name[prefix_start:prefix_end], prefix_start, newline


def _keyword_password_multiline_prefix(name, marker, newline):
    ...
    while line_start < marker:
        ...
        if next_newline == -1:
            if marker - line_start > 1:
                return name[line_start:marker - 1], line_start
            return None
        if next_newline > line_start:
            return name[line_start:next_newline], line_start
        line_start = next_newline + 1
    return None


def _keyword_password_prefix(name, marker):
    prefix, match_start, newline = _keyword_password_prefix_bounds(name, marker)
    if prefix:
        return prefix.strip('. '), match_start
    if newline == -1:
        return None
    multiline = _keyword_password_multiline_prefix(name, marker, newline)
    if multiline is None:
        return None
    prefix, match_start = multiline
    return prefix.strip('. '), match_start
...
def _keyword_password_at(name, marker):
    if not marker or not name[marker - 1].isspace():
        return None
    password = _keyword_password_value(name, marker)
    if password is None:
        return None
    prefix = _keyword_password_prefix(name, marker)
    if prefix is None:
        return None
    return prefix[0], password, prefix[1]


def _keyword_password_cross_equals(name, last_line_start):
    before_final = last_line_start - 1
    while before_final >= 0 and name[before_final].isspace():
        before_final -= 1
    on_final = last_line_start
    while on_final < len(name) and name[on_final].isspace():
        on_final += 1
    if before_final >= 0 and name[before_final] == '=':
        return before_final, on_final
    if on_final < len(name) and name[on_final] == '=':
        return on_final, on_final
    return None, on_final


def _keyword_password_first_line_candidate(name, last_line_start, marker):
    has_earlier_prefix = any(
        name[index] != '\n'
        for index in range(max(0, last_line_start - 1))
    )
    if not has_earlier_prefix:
        return None
    if not _is_password_keyword(name, marker):
        return None
    result = _keyword_password_at(name, marker)
    if not result:
        return None
    return result[2], -marker, result[:2]


def _keyword_password_cross_line_results(name, last_line_start):
    cross_equals, on_final = _keyword_password_cross_equals(name, last_line_start)
    results = []

    first_line_candidate = _keyword_password_first_line_candidate(
        name, last_line_start, on_final,
    )
    if first_line_candidate:
        results.append(first_line_candidate)

    if cross_equals is None:
        return results
    keyword_end = cross_equals
    while keyword_end and name[keyword_end - 1].isspace():
        keyword_end -= 1
    marker = keyword_end - len('password')
    if _is_password_keyword(name, marker):
        result = _keyword_password_at(name, marker)
        if result:
            results.append((result[2], -marker, result[:2]))
    return results


def _keyword_password_on_final_line(name, last_line_start):
    keyword_length = len('password')
    for marker in range(len(name) - keyword_length, last_line_start - 1, -1):
        if _is_password_keyword(name, marker):
            result = _keyword_password_at(name, marker)
            if result:
                return result[:2]
...
def _keyword_password(name):
    ...
    # newline.
    cross_line_results = _keyword_password_cross_line_results(
        match_name, last_line_start,
    )
    ...
    return _keyword_password_on_final_line(match_name, last_line_start)
