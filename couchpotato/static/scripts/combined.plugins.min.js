...
var PutIODownloader = new Class({
    ...
    addRegisterButton: function() {
        ...
        var setting_page = App.getPage("Settings");
        setting_page.addEvent("create", function() {
            if (!setting_page.tabs.downloaders || !setting_page.tabs.downloaders.groups.putio) return;
...
MA.Trailer = new Class({
    ...
    getDetails: function() {
        ...
        if (!self.player_container) {
            ...
            requestTimeout(function() {
                var identifier = self.get("imdb") || self.get("identifier");
                var hideTrailer = function() {
                    var section = self.container.getParent(".section_trailer") || self.container.getParent(".section");
                    if (section) section.addClass("no_trailer");
                    self.container.setStyle("display", "none");
                };
                if (!identifier) { hideTrailer(); return; }
                new Request.JSON({
                    url: Api.createUrl("movie.trailer") + "?identifier=" + identifier,
                    onSuccess: function(json) {
                        if (json && json.success && json.video_id) {
                            self.video_id = json.video_id;
                            self.background.setStyle("background-image", "url(" + json.thumbnail + ")");
    ...
    watch: function() {
        var self = this;
        if (!self.video_id) return;
        var yt_url = "https://www.youtube.com/watch?v=" + self.video_id;
        if (!window.YT || !window.YT.Player) {
            var tag = document.createElement("script");
            tag.src = "https://www.youtube.com/iframe_api";
            document.head.appendChild(tag);
        }
        var tryPlayer = function() {
            if (!window.YT || !window.YT.Player) { requestTimeout(tryPlayer, 100); return; }
            new YT.Player(self.id, {
                videoId: self.video_id,
                playerVars: { autoplay: 1, rel: 0, iv_load_policy: 3, vq: "hd720" },
                events: {
                    onError: function() {
                        self.player_container.set("html", "");
                        self.player_container.adopt(
                            new Element("div.trailer_error").adopt(
                                new Element("p[text=Trailer can't be embedded]"),
                                new Element("a.trailer_fallback[href=" + yt_url + "][target=_blank][text=Watch on YouTube ↗]")
                            )
                        );
                    }
                }
...
MA.WatchToggle = new Class({
    ...
    createButton: function() {
        var self = this, watched = self.movie.get("watched");
...
var Movie = new Class({
    ...
    create: function() {
        ...
        self.inner.adopt(self.select_checkbox = new Element("input[type=checkbox]"), new Element("div.poster_container").adopt(thumbnail, self.actions_el = new Element("div.actions")), new Element("div.info").adopt(new Element("div.title").adopt(new Element("span", {
            ...
            text: eta_date,
            title: "ETA"
        }) : null, self.get("watched") ? new Element("div.watched_status", {
            text: "Watched",
            ...
            self.list = new MovieList({
                ...
                on_empty_element: function() {
                    return new Element("div.empty_wanted").adopt(new Element("div.no_movies", {
                        text: "Seems like you don't have any movies yet.. Try adding some via search or automation."
...
var TraktAutomation = new Class({
    ...
    addRegisterButton: function() {
        ...
        setting_page.addEvent("create", function() {
            if (!setting_page.tabs.automation || !setting_page.tabs.automation.groups.trakt_automation) return;
...
var TwitterNotification = new Class({
    ...
    addRegisterButton: function() {
        ...
        var setting_page = App.getPage("Settings");
        setting_page.addEvent("create", function() {
            if (!setting_page.tabs.notifications || !setting_page.tabs.notifications.groups.twitter) return;
...
Page.Wizard = new Class({
    ...
    headers: {
        ...
        automation: {
            ...
            description: "Add movies from IMDB, Trakt, Letterboxd or other automation sources, or use the search bar to find and add movies directly.",
            content: function() {
        ...
        finish: {
            title: "Finishing Up",
            description: "Are you done? Did you fill in everything as much as possible?" + "<br />Be sure to check the settings to see what more CP can do!<br /><br />" + '<div class="wizard_support">If you find issues or want to contribute, visit the <a href="https://github.com/bassings/CouchPotatoServer" target="_blank">GitHub repo</a>.</div>',
