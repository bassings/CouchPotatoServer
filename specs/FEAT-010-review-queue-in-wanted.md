...
- **AC-SEC-7:** Mark Done refuses a STALE card, and only a stale one. `media.done` accepts an optional `expected_status`; when supplied and it does not match the stored status, the call performs no write and returns failure, evaluated inside the existing `update_with_retry` mutator so it is atomic against the freshly re-read document rather than a check-then-write. When omitted, behaviour is exactly as today. Both card controls supply it. Proven in four directions: supplied and matching succeeds; supplied and stale performs no write and leaves the stored status untouched; omitted still succeeds (the pre-existing callers); and a movie whose status is `active` marked done WITH `expected_status='active'` still succeeds, which is the FEAT-001 case below.

  **AMENDED 2026-08-30, before implementation, and this is a spec bug in a criterion I added at synthesis.** The original said "calling the route for a movie whose status is `active` returns failure", which reads as a hard rule that `media.done` only accepts a `downloaded` movie. That would have **broken FEAT-001**: `movie_detail.html:261` offers "Mark as Done" for every status except `done` and `downloaded`, and FEAT-001 exists precisely to "move it out of the active Wanted workflow", so marking an `active` movie done is intended, shipped behaviour with its own spec. An implementer following the original wording would have written a guard that silently removed a feature, and the guard would have looked correct and passed its own tests. The stale-card risk the criterion was reaching for is optimistic concurrency, not a status allowlist, and the two only coincide on the card path. Recorded rather than quietly rewritten, because the same confusion is what put `_reset_if_downloaded`'s scope guard and this one in the same sentence in the first place.

  *(Note for the implementer: `movie.searcher.mark_failed` needs NO new code for AC-SEC-3. `markFailedAndResearch` already refuses any non-`downloaded` movie atomically via `_reset_if_downloaded` at `searcher.py:817-824`. That AC is a regression pin plus the unauthenticated half, and writing a second guard there would be duplication, not safety.)*
