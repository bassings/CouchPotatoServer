...
2. **Card actions.** Only **Mark Done** goes on the card. **Mark Failed stays on
   the detail page only** (see "Vetoed at planning").
...
- **AC-SIMP-4:** No destructive action is added to `couchpotato/ui/templates/partials/movie_cards.html`: the diff of that file contains no reference to `movie.searcher.mark_failed`, `media.delete` or `confirm(`. The only route to Mark Failed remains the movie detail page, where the deliberate navigation and the existing `confirm()` (`movie_detail.html:281`) are both intact and unmodified. *(This is the mechanical form of the Mark Failed veto below, and it subsumes lens-security's AC-SEC-3 and lens-design's AC-DESIGN-8.)*
...
|---|---|---|
| **Mark Failed on a Wanted card** (`movie_cards.html` row of Affected files), and with it AC-SEC-3, AC-QA-12, AC-QA-13, AC-DESIGN-8 | vetoed by `lens-simplicity`, upheld | The spec's stated cost is visibility only ("a downloaded film is indistinguishable from a lost one"). Mark Failed already works from the detail page, so the card button buys one saved click while moving the one action that discards a completed download from behind a deliberate navigation to a hover target on a dense grid, the spec's own highest-harm risk, made cheaper to trigger. Precedence rule 1 (irrecoverable-adjacent loss) reinforces the veto rather than overriding it. AC-SIMP-4 enforces the drop mechanically, so removing the guards leaves no gap. **Reinstate AC-SEC-3, AC-QA-12 and AC-QA-13 verbatim if card-level Mark Failed is ever added.** |
