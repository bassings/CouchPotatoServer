...
- **AC-QA-12** (amended during B4a review, see D9): When `release.for_media` returns an empty list, returns `INCOMPLETE_RELEASE_SET`, raises, or the media dict has no `_id`, the renamer refuses, does not propagate an exception out of `_moveRenamedFiles`, and leaves the library file and the download intact. The recorded outcome names the CAUSE and the four are distinct: `declined_no_owner` (empty list, or no `_id` -- nothing claims this destination), `declined_incomplete_evidence` (the sentinel -- a release document exists but could not be read), `declined_error` (raised), and `declined_unknown_quality` (the identifier is not on the ranking ladder -- a fourth, genuinely different cause, see D9). Proof: unit.
