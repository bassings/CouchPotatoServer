...
- **AC-QA-12**: When `release.for_media` returns an empty list, raises, or the media dict has no `_id`, the renamer refuses, does not propagate an exception out of `_moveRenamedFiles`, leaves the library file and the download intact and records `declined_unknown_quality`. Proof: unit.
