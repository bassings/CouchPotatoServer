...
test.describe('Movie Detail', () => {
  // scripts/seed_e2e_data.py always seeds at least one 'active' movie, so
  // the Wanted grid always has a first card to click through -- the
  // `if (await firstCard.count() > 0)` guard each of the next five tests
  // used to have could effectively never be false, and hid what would
  // otherwise be zero assertions if it somehow were. Fail loudly instead
  // (matching gotoMovie's "FAIL, don't skip" above) rather than silently
  // asserting nothing.
  async function gotoFirstWantedCard(page: Page) {
    await page.goto('/');
  ...
  test('should navigate to movie detail from wanted list', async ({ page }) => {
    const firstCard = await gotoFirstWantedCard(page);
  ...
  test('Mark Failed & Re-search requires confirmation when shown (review-gate movie)', async ({ page }) => {
    ...
    // way to reliably produce a 'downloaded' movie, so the button is
    // structurally never shown here today. Both branches are asserted
    // (vacuous-guard-ok, T1.4/AC-QA-42) rather than only the "shown" one,
    // so this cannot silently pass by asserting nothing when it doesn't
    ...
    if (await markFailedButton.count() > 0) { // vacuous-guard-ok: structurally unreachable without a 'downloaded'-status fixture (see coverage-gap comment above); the else branch below asserts the always-true case for real.
      ...
      await expect(markFailedButton).toHaveCount(0);
    }
  });
  ...
  test('per-release Mark failed requires confirmation when shown', async ({ page }) => {
    ...
    // in this suite, so this is structurally unreachable today; the else
    ...
    if (await releaseMarkFailedButton.count() > 0) { // vacuous-guard-ok: structurally unreachable without a 'downloaded'-status release fixture (see coverage-gap comment above); the else branch asserts the always-true case.
      ...
      await expect(releaseMarkFailedButton).toHaveCount(0);
    }
  });
