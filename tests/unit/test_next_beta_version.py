...
(AC1-AC9), asserting the exact expected output string for each tag-list input.
"""
import subprocess
import sys
...
def test_ac3_rerun_on_existing_beta_increments_suffix_not_minor():
    assert next_beta_version(['v3.9.1', 'v3.10.0-beta.1']) == '3.10.0-beta.2'


def test_ac4_rerun_on_multiple_existing_betas_increments_from_max_suffix():
    assert next_beta_version(['v3.9.1', 'v3.10.0-beta.1', 'v3.10.0-beta.2']) == '3.10.0-beta.3'
...
def test_ac9_cli_reads_tags_from_stdin_when_no_argv_given():
    ...
    assert result.stdout == '3.10.0-beta.2\n'
