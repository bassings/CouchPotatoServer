...
**Exception -- a badge sitting directly over poster artwork** (`partials/movie_cards.html`'s downloaded/review badge): use solid `bg-cp-warning text-black`, never the translucent `/20` form above. A tint sharing its hue with its own text cannot reach the WCAG 1.4.3 4.5:1 floor at any opacity once real poster artwork is behind it (measured 1.92:1 against both a black and a white poster fixture) -- solid is opaque enough that the poster stops mattering (9.4:1, theme-independent). Any badge placed over artwork rather than a flat surface follows this exception, not the translucent default above.
